...
plot_tracking_freq = function(cycles, goal, sex){
  ...
  plot(h$mids,f, type = 'n', axes = FALSE, xlim = c(0,1), ylim = range(0,0.45),yaxs = 'i', xaxs = 'i',
       ...
       main = paste(goals.dict$goals[goals.dict$index ==  goal],'|', sex,'|',length(x)))
  ...
  rect(breaks[1:(N-1)],rep(0,N-1),breaks[2:N],f, col = goals.dict$colors[goals.dict$index == goal], border = NA)
...
#                      cycles = cycles, 
  ...
  points(this.cycle$out_cycleday, rep(line.blood,n.days), 
         ...
         col = bleeding.dict$colors[match(this.cycle$blood,bleeding.dict$index)])
  ...
  points(this.cycle$out_cycleday, rep(line.mucus,n.days), 
         pch = mucus.dict$symbols[match(this.cycle$elixir,mucus.dict$index)], 
         cex = mucus.dict$cex[match(this.cycle$elixir,mucus.dict$index)], 
         col = mucus.dict$colors[match(this.cycle$elixir,mucus.dict$index)])
  ...
  points(this.cycle$out_cycleday, rep(line.cervix, n.days),
         pch = cervix.dict$symbols[match(this.cycle$feel,cervix.dict$index)],
         cex = cervix.dict$cex[match(this.cycle$feel,cervix.dict$index)],
         col = cervix.dict$colors[match(this.cycle$feel,cervix.dict$index)])
  ...
  points(this.cycle$out_cycleday, rep(line.sex,n.days), 
         pch = 25, cex = 2, 
         col = sex.dict$colors[match(this.cycle$sex,sex.dict$index)], 
         bg = sex.dict$bg[match(this.cycle$sex,sex.dict$index)])
  ...
  if(length(days.with.tmp)>0){
    ...
    tct$adj.temp = adj.tmp.fn(tct$temp_c,rt,margin.temp)
    
    grid.value = seq(0.1*ceiling(10*rt[1]),0.1*floor(10*rt[2]),by = 0.1)
    adj.grid.value = adj.tmp.fn(grid.value, rt, margin.temp)
  ...
  points(this.cycle$out_cycleday+0.5,rep(0,n.days),
         ...
         bg = js.dict$colors[match(this.cycle$out_js,js.dict$index)])
  
  #breast feeding
  points(this.cycle$out_cycleday+0.5,rep(0,n.days),
         pch = 17,cex = 2,
         col = bf.dict$colors[match(this.cycle$bf,bf.dict$index)])
  ...
  return(list(rt= rt, margin.temp = margin.temp))
}
...
plot_fit_results = function(cycletable = cycletable, opt = opt, publi = TRUE) {
  ...
  if(opt$do.temp.fit){
    ...
    if(length(days.with.tmp)>0){
      ...
      y = adj.tmp.fn(temp.curve.optimized$temp,rt,margin.temp) 
      ...
      # plot the curve
      points(time.vect, 
             adj.tmp.fn(temp.curve.optimized$temp,rt,margin.temp), 
             col = 'blue', type = 'l')
    }
...
plot.tracking.history = function(d = cycledays, 
  ...
  if(show_goal){col =  goals.dict$colors[match(d$goal, goals.dict$goals)]}
  ...
  points(d$obs_day, rep(n.lines, nrow(d))+(d$temp_c-min.temp)*2,
         type = 'l', col = 'gray90')
  temp.col = temp.dict$colors[as.numeric(cut(d$temp_c, temp.dict$values))]
  points(d$obs_day, rep(n.lines, nrow(d))+(d$temp_c-min.temp)*2,
  ...
  #       cex =  d$blood/2, 
  #       col = bleeding.dict$colors[match(d$blood, bleeding.dict$index)] )
  ...
  points(d$obs_day, rep(line.mucus, nrow(d))+d$elixir/5, pch = 16, 
         cex =  mucus.dict$cex[match(d$elixir, mucus.dict$index)]/4,
         col = mucus.dict$colors[match(d$elixir, mucus.dict$index)]
  )
  ...
  points(d$obs_day, rep(line.cervix, nrow(d))+(d$feel%%10)/5, pch = 21, 
         cex =  (d$feel%%10)/3,
         col = cervix.dict$colors[match(d$feel, cervix.dict$index)]
  )
  ...
  points(d$obs_day, rep(line.sex, nrow(d))+d$sex/tot.sex,
         cex =  0.5,
         pch = sex.dict$symbols[match(d$sex, sex.dict$index)],
         col = sex.dict$colors[match(d$sex, sex.dict$index)],
         bg = sex.dict$bg[match(d$sex, sex.dict$index)]
  )
