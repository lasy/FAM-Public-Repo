...
j = which((CYCLES$confidence >= 0.8) & (CYCLES$ovu.sd <= 0.5))

for(i in j){
  k = which(days$cycle_id == CYCLES$cycle_id[i])
  cat(CYCLES$cycle_id[i],'\n')
  plot_fit_results_hmm(cycletable = days[k,],cycles = CYCLES, hmm.res = NA)
...
cycle_ids = c('1497e11dfe844e86b2c84ec11f557af3_4',
              ...
              'fc38e35706474f87816d4f31c1ff78c8_29' # goal is wrong for this one
              )
...
for(cycle_id in cycle_ids){
  k = which(days$cycle_id == cycle_id)
  pdf(paste0(plot_path_fig4,'B_fit_cycle_example',cycle_id,'.pdf'), 
      useDingbats = FALSE, width = pdf.full.width/2, height = 5)
